...
class B3A1 {
    ...
    public static void printSubarrays(int[] array, int k) {
        System.out.println(); // Print a new line for better readability
        printSubarrays(array, k, 0, "");
        System.out.println(); // Formatting output
    }

    // Overloaded recursive function to print all subarrays of length k
    private static void printSubarrays(int[] array, int k, int start, String current) {
        if (k == 0) {
        ...
        for (int i = start; i <= array.length - k; i++) {
            ...
            printSubarrays(array, k - 1, i + 1, newCurrent);
        }
    }
